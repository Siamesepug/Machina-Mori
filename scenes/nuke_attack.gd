...
@onready var damage = player.weapon_damage * 100
@onready var camera = $"../../Camera2D"

@onready var collision_shape = $CollisionShape2D
@onready var beam_timer = $NukeCD
@onready var beam_light = $"../NukeLight"
@onready var beam_audio = $"../NukeAudio"
...
func activate_nuke():
	...
	damage = player.weapon_damage * 100
	beam_duration = PlayerAttributes.beam_duration
	
	# animation for starting laser
	var tween_start = create_tween()
	
	tween_start.tween_property(beam_sprite.material, "shader_parameter/progress", 1.0, 1.0)
	...
	tween_end.tween_property(beam_sprite.material, "shader_parameter/progress", 0.0, 1.0)
